...
## 📝2021-3
    - 정보 보호 기술 AAA
        ...
        - Accounting : 계정
    - OSI 7 계층
        - 물리 : 데이터를 전기 신호로 변환 On/off
        - 데이터링크 : 물리계층을 통해 송수신되는 정보의 오류 & 흐름 제어/ MAC 주소로 통신
        - 네트워크 : 경로를 찾아 데이터 전송, IP, Route 패킷
        - 전송 : TCP 프로토콜 / 전송 속도 조절, 오류 찾기
        - 세션 : 네트워크의 양쪽 연결 , TCP/IP 삭제, 생성
        - 표현 : 데이터 인코딩 및 디코딩 / JPEG,...
        - 응용 : 사용자가 네트워크 접근할 수 있도록 도움, 사용자가 볼 수 있는 유일한 계층
    - 암호화 (양방향(대칭 / 비대칭), 일방향)
        - 양방향 : 대칭키 (암, 복호화 키 동일)
            - 블록암호 (DES,AES,SEED,ARIA)
                - DES : 64비트 평문을 64비트 암호문으로, 미국표준
            - 비대칭 키: 공개키 , 개인키 존재(암 != 복)
                - RSA(소인수 분해 사용)
        - 단방향 : 해시 -> 레인보우 테이블 공격에 취약
            - MAC, MDC
---
...
<<(좌 시프트)
>>(우 시프트)
ex) a=5, b=7 --> a = 101 , b=111
a&b = 5 (101), a^b = 2(010), a|b = 7(111), ~b = -8
a>> 1 : 2(01) , b<<3 : 56(111000)
```